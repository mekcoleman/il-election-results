#!/usr/bin/env python3
"""
McDonough County Election Results Scraper

Scrapes primary election results from McDonough County's custom web platform.
Supports multiple formats: HTML, PDF, and text-based results.

County: McDonough County, Illinois (~20,000 voters)
Location: West-central Illinois (Macomb, Western Illinois University)
Platform: Custom web-based system (Logonix Corporation) at mcdonoughelections.com
...
class McDonoughCountyScraper:
    """Scraper for McDonough County election results."""
    ...
    def scrape(self) -> Dict:
        """
        Scrape election results from McDonough County.
    ...
    def _show_instructions(self) -> Dict:
        ...
        instructions = """
╔══════════════════════════════════════════════════════════════════════════════╗
║                  MCDONOUGH COUNTY SCRAPER - SETUP REQUIRED                   ║
╚══════════════════════════════════════════════════════════════════════════════╝

To use this scraper, you need to provide the URL to McDonough County's results.

STEP 1: Find the Results
────────────────────────────────────────────────────────────────────────────────
Visit: https://www.mcdonoughelections.com

Look for:
  • "Election Results" link
  • "2026 Primary Election Results"
  • "View Results" or "Results" button
  • Current election results page
...
The URL might look like:
  ...
  • https://www.mcdonoughelections.com/files/2026-primary-results.pdf
  • Or similar format

STEP 3: Run the Scraper
────────────────────────────────────────────────────────────────────────────────
python mcdonough_county_scraper.py --url "YOUR_URL_HERE"
...
SUPPORTED FORMATS:
────────────────────────────────────────────────────────────────────────────────
  ✓ HTML results pages (Logonix platform)
...
COLLEGE TOWN NOTE:
────────────────────────────────────────────────────────────────────────────────
McDonough County includes Western Illinois University (Macomb). Results may
show separate student/university precincts. The scraper handles all precincts.

NEED HELP?
────────────────────────────────────────────────────────────────────────────────
See MCDONOUGH_COUNTY_SETUP.md for detailed instructions and troubleshooting.
    ...
    def _scrape_pdf(self) -> Dict:
        ...
        try:
            ...
            # Save temporarily
            pdf_path = "/tmp/mcdonough_results.pdf"
            ...
            results = {
                "county": "McDonough",
                "election_date": self.election_date,
                "source": "McDonough County Elections PDF",
                "pdf_url": self.url,
                "scraped_at": datetime.now().isoformat(),
                "metadata": {},
                "contests": []
    ...
    def _parse_html_contests(self, soup: BeautifulSoup) -> List[Dict]:
        """Parse contests from HTML tables or structured content."""
        contests = []
        ...
        if tables:
            contests.extend(self._parse_tables(tables))
        
        # Strategy 2: Look for divs with contest classes
        contest_divs = soup.find_all('div', class_=re.compile(r'contest|race|election', re.I))
        if contest_divs:
            contests.extend(self._parse_contest_divs(contest_divs))
        
        # Strategy 3: Look for structured lists
        if not contests:
            contests.extend(self._parse_structured_lists(soup))
    ...
    def _parse_contest_divs(self, divs: List) -> List[Dict]:
        """Parse contests from structured div elements."""
        contests = []
        
        for div in divs:
            # Try to find contest name
            contest_name = None
            heading = div.find(['h1', 'h2', 'h3', 'h4', 'h5'])
            if heading:
                contest_name = heading.get_text().strip()
            ...
            candidates = []
            candidate_items = div.find_all(['li', 'div'], class_=re.compile(r'candidate|choice', re.I))
    ...
    def _parse_structured_lists(self, soup: BeautifulSoup) -> List[Dict]:
        """Parse contests from structured list elements."""
        contests = []
        
        # Look for definition lists or unordered lists with results
        lists = soup.find_all(['dl', 'ul', 'ol'])
        
        for lst in lists:
            # This is a fallback parser - implementation depends on actual structure
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Scrape McDonough County election results',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # With results URL
  %(prog)s --url "https://www.mcdonoughelections.com/results/2026-primary.html"
  
  # Test with historical data
  %(prog)s --url "https://www.mcdonoughelections.com/results/2024-primary.html" --date 2024-03-19
  
  # Save to specific file
  %(prog)s --url "URL" --output mcdonough_results.json
        """
    )
    ...
    parser.add_argument(
        '--output',
        help='Output JSON file (default: mcdonough_results.json)'
    )
    
    args = parser.parse_args()
    
    # Create scraper
    scraper = McDonoughCountyScraper(
        url=args.url,
        election_date=args.date
    )
    
    # Scrape results
    print("Scraping McDonough County election results...")
    if args.url:
    ...
    if args.output:
        ...
        output_file = "mcdonough_results.json"
    ...
    if "error" in results:
        ...
        print(f"✓ Successfully scraped McDonough County results")
